...
public partial class NDeleteButton : NButton
{
    ...
    public override void _Ready()
    {
        ...
        {
            Name = "DeleteButtonImage",
            Texture = GD.Load<Texture2D>(Path.Join(MainFile.ModId, "delete_button.png")),
    ...
    protected override void OnRelease()
    {
        ...
        {
            string path = Path.Combine(Config.CustomStampDir, _stampPickerButton.Stamp.Name + ".json");
            if (File.Exists(path))
            {
                File.Delete(path);
                CustomStampLoader.LoadStamps(Config.CustomStampDir);
                _stampPickerScreen.Rebuild();
            }

            if (_stampPickerButton.IsSelected)
            {
                StampRegistry.SetActive(null);
                _stampPickerButton.EmitSignal(NStampPickerButton.SignalName.StampUnSelected, _stampPickerButton);
            }
        }

        if (_stampDisplayTile is not null)
        {
            string path = Path.Combine(Config.CustomStampDir, _stampDisplayTile.Stamp.Name + ".json");
            if (File.Exists(path))
            {
                File.Delete(path);
                CustomStampLoader.LoadStamps(Config.CustomStampDir);
                _configStampPreviewControl.Reload();
